...
@endsection
 ...
 <form action="{{ route('notice.update',$notice->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
 <input type="text" name="priority" placeholder="Enter Priority" class="w-full rounded-lg mt-4 p-2" value="{{ $notice->priority}}">
        @error('priority')
            <p class="text-red-500 text-xs mt-0">*{{$message}}</p>
        @enderror
        <input type="text" name="notice_title" placeholder="Enter title" class="w-full rounded-lg mt-4 p-2" value="{{ $notice->notice_title}}">
        @error('notice_title')
            <p class="text-red-500 text-xs mt-0">{{$message}}</p>
        @enderror
        <input type="date" name="notice_date" placeholder="Enter date" class="w-full rounded-lg mt-4 p-2" value="{{ $notice->notice_date}}">
        @error('notice_date')
            <p class="text-red-500 text-xs mt-0">{{$message}}</p>
        @enderror
